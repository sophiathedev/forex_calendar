defmodule ForexCalendarWeb.UserServerSettingsLive do
  ...
  def render(assigns) do
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="mb-6">
        ...
        >
          <.icon name="hero-arrow-left" class="h-4 w-4" /> Back to Servers
        </.link>
      </div>
              ...
              <div class="flex items-center justify-between">
                <div>
                  <h3 class="text-sm font-medium text-yellow-800">
                    Channel Actions for <span class="font-bold">{@server_channel.name}</span> channel
                  </h3>
                ...
                >
                  <.icon name="hero-trash" class="h-4 w-4 mr-2" /> Bulk Delete Messages
                </.button>
              </div>
            </div>
          <% end %>
  ...
  def mount(%{"server_id" => server_id}, _session, socket) do
    ...
    if server.user_id != user.id do
      ...
      server_setting =
        Servers.get_server_settings(server.id) || %ServerSetting{server_id: server.id}
      ...
      server_setted_channel =
        if server_setting_channel_id,
          do: get_discord_guild_channel(server_setting_channel_id),
          else: nil
