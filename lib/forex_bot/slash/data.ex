defmodule ForexBot.Slash.Data do
  ...
  ] |> Enum.map(fn name -> %{label: name, value: name} end)

  @impl true
  def description, do: "Lấy dữ liệu từ 1 đến 12 tháng gần nhất"
  ...
  def command(interaction) do
    ...
    if trunc(month_to_cpi) < 1 or trunc(month_to_cpi) > 12 do
      ...
      {:ok, true} = Cachex.put(:cache, "interaction:data:#{interaction.id}", %{month: trunc(month_to_cpi)}, expire: :timer.hours(1))

      selection_menu = select_menu("event_for_data", options: @allowed_event_names, placeholder: "Lựa chọn loại sự kiện", min_values: 1, max_values: 1)

      action_row = action_row(components: [selection_menu])
